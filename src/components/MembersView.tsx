        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          ...
          >
            ...
            <input
              ...
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="px-6 py-4 rounded-3xl text-sm font-bold focus:outline-none min-w-[240px]"
              style={{
                background: "var(--bg)",
                color: "var(--text)",
                border: "1px solid var(--border)",
            ...
            <select
              value={domain}
              onChange={(e) => setDomain(e.target.value as Domain)}
              className="px-6 py-4 rounded-3xl text-sm font-bold focus:outline-none"
              style={{
                background: "var(--bg)",
                color: "var(--text)",
                border: "1px solid var(--border)",
            ...
            <button
              type="submit"
              className="px-10 py-4 rounded-3xl text-sm font-black flex items-center gap-2"
      ...
      {members.length > 0 ? (
        <div className="flex flex-wrap gap-4">
          {members.map((member, idx) => (
            <div
              key={member.id}
              className="group flex items-center gap-4 pl-4 pr-1 py-1 rounded-full border shadow-sm transition-all"
              style={{
                background: "var(--card)",
                borderColor: "var(--border)",
            ...
            >
              ...
              {/* Info */}
              <div className="flex flex-col min-w-[100px] py-2">
                <h3 className="font-black text-sm text-[var(--text)] truncate">
                  {member.name}
                </h3>
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-red-500 hover:text-white transition"
              >
                <Trash2 size={16} />
              </button>
            </div>
        ...
        <div
          className="py-32 text-center rounded-[3.5rem] border-4 border-dashed"
          style={{ borderColor: "var(--border)" }}
        >
          <UserPlus className="mx-auto mb-6 opacity-40" size={32} />
          <h3 className="text-xl font-black opacity-60">Empty Registry</h3>
